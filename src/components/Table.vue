...
export default {
  ...
  methods: {
    ...
    handleResize() {
      const { states } = this.store;
      this.excelPos = this.$refs.excelTable && this.$refs.excelTable.getBoundingClientRect();
    ...
    multiSelectAdjustPostion(e) {
      ...
      if (states.selector.isSelected || states.autofill.isAutofill) {
        ...
        const sTop = this.$refs.excelTable.offsetTop + this.theaderHeight;
        const sLeft = this.excelPos.left + this.fixedWidth;
        const sBottom = this.$refs.excelTable.offsetTop + this.$refs.excelTable.offsetHeight - 10;
        const sRight = this.excelPos.left + this.$refs.excelTable.offsetWidth - 10;
        let { tableBodyTop } = states;
        let { tableBodyLeft } = states;
